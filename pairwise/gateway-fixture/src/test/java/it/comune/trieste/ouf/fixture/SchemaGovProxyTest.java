...
class SchemaGovProxyTest {
  ...
  @Test void fetchesOnlyAllowedRdf(){upstream.createContext("/rdf",x->{try{byte[] b="<urn:a> <urn:p> <urn:b> .".getBytes();x.getResponseHeaders().add("Content-Type","text/turtle");x.sendResponseHeaders(200,b.length);x.getResponseBody().write(b);x.close();}catch(Exception e){throw new RuntimeException(e);}});assertThat(proxy.fetch(base.resolve("/rdf").toString(),"trace").mediaType()).isEqualTo("text/turtle");}
