...
public class ValidationApi {
  ...
  @PostMapping("/revisions/{id}:validate") Map<String,Object> validate(@PathVariable UUID id,HttpServletRequest request){return service.validate(id,actors.actor(request).subject());}
  @GetMapping("/validation-runs/{id}") Map<String,Object> validation(@PathVariable UUID id){return service.validation(id);}
  @PostMapping("/impact-reports") Map<String,Object> impact(@RequestBody ImpactRequest r,HttpServletRequest request){return service.impact(r.fromRevisionId(),r.toRevisionId(),actors.actor(request).subject());}
