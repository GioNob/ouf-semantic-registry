...
public class ValidationApi {
  ...
  @SemanticCapability("ouf.semantic.review.prepare") @PostMapping("/revisions/{id}:validate") Map<String,Object> validate(@PathVariable UUID id,HttpServletRequest request){return service.validate(id,actors.actor(request).subject());}
  ...
  @SemanticCapability("ouf.semantic.review.prepare") @PostMapping("/impact-reports") Map<String,Object> impact(@RequestBody ImpactRequest r,HttpServletRequest request){return service.impact(r.fromRevisionId(),r.toRevisionId(),actors.actor(request).subject());}
