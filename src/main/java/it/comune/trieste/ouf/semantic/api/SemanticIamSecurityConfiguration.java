...
public class SemanticIamSecurityConfiguration {
  @Bean
  ...
  Converter<Jwt,? extends AbstractAuthenticationToken> trustedJwtAuthenticationConverter(
    ...
    return jwt->{
      ...
      try{actorType=PrincipalContext.ActorType.valueOf(actorText);}
      catch(IllegalArgumentException e){throw new IllegalArgumentException("invalid OUF actor type",e);}
      ...
      var claims=new PrincipalContext.IdentityClaims(Set.of(),acr,amr,authenticatedAt);
  ...
  @Bean
  SecurityFilterChain semanticIamSecurityFilterChain(
      HttpSecurity http,
  ...
  private static Set<String> stringSet(Object value){
    ...
    if(value instanceof String s){
      ...
      for(String item:s.trim().split("\\s+"))if(!item.isBlank())out.add(item);
      return Set.copyOf(out);
    }
    if(value instanceof Collection<?> values){
      var out=new LinkedHashSet<String>();
      for(Object item:values){String t=text(item);if(t!=null)out.add(t);}
      return Set.copyOf(out);
    }
    return Set.of();
  }
  ...
  static final class TrustedJwtAuthenticationToken extends AbstractAuthenticationToken {
    private final Jwt jwt;private final TrustedPrincipal principal;
...
class SemanticIamDisabledSecurityConfiguration {
  @Bean
  SecurityFilterChain semanticIamDisabledSentinelSecurityFilterChain(HttpSecurity http) throws Exception {
