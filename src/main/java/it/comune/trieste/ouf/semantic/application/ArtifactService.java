...
public class ArtifactService {
  ...
  public Map<String,Object> create(CreateArtifact c){
    ...
    try {
      ...
      db.sql("insert into ouf_sem.artifact_revision(revision_id,artifact_id,revision_no,lifecycle_status,semantic_version,label_json,definition_json,created_by_subject) values(:r,:a,1,'DRAFT',:v,cast(:l as jsonb),cast(:d as jsonb),'api')")
        .param("r",r).param("a",a).param("v",c.semanticVersion()).param("l",j(c.labels())).param("d",j(c.definition())).update();
  ...
  @Transactional public Map<String,Object> patch(UUID id,String etag,PatchRevision p){
    final long expected;
    ...
    try{
      ...
      return Map.of("revisionId",id,"etag","\""+v+"\"");
