...
public class SemanticTrustedHumanApi {
  ...
  @PostMapping("/revisions/{id}:deprecate") Map<String,Object> deprecate(@PathVariable UUID id,HttpServletRequest request,@RequestHeader HttpHeaders headers){var actor=human(request);actors.require(actor,"ouf.semantic.deprecate");return governance.lifecycle(id,"DEPRECATED",actor.subject(),actor.type(),actors.authorizationContextRef(request),correlation(headers));}
  @PostMapping("/artifacts/{id}:retire") Map<String,Object> retire(@PathVariable UUID id,HttpServletRequest request,@RequestHeader HttpHeaders headers){var actor=human(request);actors.require(actor,"ouf.semantic.deprecate");return governance.lifecycle(governance.retiredRevision(id),"RETIRED",actor.subject(),actor.type(),actors.authorizationContextRef(request),correlation(headers));}
  ...
  @PostMapping("/migration-proposals/{id}/decision") Map<String,Object> migrationDecision(@PathVariable UUID id,@Valid @RequestBody MigrationDecisionRequest body,HttpServletRequest request){var actor=human(request);actors.require(actor,"ouf.semantic.migration.decide");return changes.decide(id,body.decision(),actor.subject(),actor.type(),actors.authorizationContextRef(request));}
  private TrustedActorResolver.Actor human(HttpServletRequest request){var actor=actors.actor(request);if(!"HUMAN_USER".equals(actor.type()))throw new GovernanceService.Forbidden("HUMAN_IDENTITY_REQUIRED");return actor;}
