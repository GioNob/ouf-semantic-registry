...
public class ChangeGovernanceService {
  ...
  @Transactional public Map<String,Object> decide(UUID proposal,String decision,String actor,String actorType){if(!"HUMAN_USER".equals(actorType))throw new GovernanceService.Forbidden("HUMAN_IDENTITY_REQUIRED");if(!Set.of("APPROVED","REJECTED").contains(decision))throw new IllegalArgumentException("MIGRATION_DECISION_INVALID");int n=db.sql("update ouf_sem.semantic_migration_proposal set status=:s,decided_by_subject=:a,decided_at=transaction_timestamp() where migration_proposal_id=:i and status='PROPOSED'").param("s",decision).param("a",actor).param("i",proposal).update();if(n!=1)throw new GovernanceService.Conflict("MIGRATION_PROPOSAL_NOT_OPEN");return proposal(proposal);}
