...
**R4a, prova APISIX e contratto owner nella CI Gateway:** il job Docker esercita la nuova rotta con OIDC, delega, scope, firma del receipt e owner HTTP di test; include casi negati. Il job `object-search-pairwise` fa consumare al filtro Java UDP un receipt prodotto dal Lua Gateway con corpo originale e alterato, correggendo la precedente omissione del campo `client` individuata dal test tra repository. La CI UDP esercita a parte il serving governato. Resta da eseguire la prova congiunta APISIX→UDP reale su dati e bundle rappresentativi, compresi minimizzazione, cursore e `partial`, e da registrare i commit coordinati nel release lock. MCP `INACTIVE`; R-INSTALL **OPEN**.
