...
Controlli già versionati: bootstrap IAM/scopes/grant con `plan/apply/verify`; snapshot e dump DB con restore di prova; MinIO policy e test negativo; candidato Docker fermo con verifica di env/mount/image ID; rollout con readiness e rollback automatico dei container; riconciliazione del tentativo fallito per ID e log privati prima della rimozione. Il commit Onboarding `4564b942f8c6b80864c58de90f886d0d7ed90ec8` aggiunge alla CI il boot dell'immagine impacchettata con configurazione staging equivalente al deploy. Verificarne l'esito prima di segnare PASS.
