...
La [procedura MCP R4a](https://github.com/GioNob/ouf-mcp-server/blob/codex/r4a-object-search-mcp/docs/R4A_MCP_RUNTIME_ROLLOUT.md) e i relativi script sono nella PR draft #42: Dockerfile committato e CI build PASS; snapshot del runtime MCP e preparazione dei file privati PASS; dry run del rollout PASS con originale invariato. Staging controllato PASS sul lab: nuovo `ouf-mcp:r4a-fbd0e8c` pronto, originale `ouf-mcp-r4a-original` conservato; il tool HUMAN `ouf.system.status` ha restituito MCP HEALTHY, `actionRequired=false`, `partial=false`. Prossimi gate: grant HUMAN approvato in THS, prove positive/negative di ricerca tramite owner e rollback dimostrato; non eliminare il container originale. La capability resta **INACTIVE**. R-INSTALL resta **OPEN**, inclusa la parametrizzazione dell'immagine e dei binding su ambienti nuovi e la prova di installazione da zero secondo i PET.

## R4a Authorization capability catalogue — 24 settembre 2026

Il preflight di produzione ha letto `ouf-lab-authorization:14`: `urban.object.search` non è registrata nel catalogo né nel bundle ACTIVE. Registrare il descriptor owner UDP con il reconciler a manifest di [Source Onboarding](https://github.com/GioNob/ouf-source-onboarding/blob/codex/r4a-authorization-catalogue/docs/R4A_AUTHORIZATION_CATALOGUE.md) prima della nuova policy, dei grant e del collaudo positivo. Il reconciler gestisce manifest multipli, differenze semantiche e paginazione; l'API paginata è ancora in PR/CI e va distribuita e collaudata prima dell'uso su cataloghi grandi. Registrazione, pubblicazione del bundle e grant rimangono tre azioni distinte con autorità HUMAN e audit. R-INSTALL rimane aperto fino a CI/CD, registrazione batch e installazione da zero ripetibile.
